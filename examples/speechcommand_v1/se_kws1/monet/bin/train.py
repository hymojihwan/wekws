...
def train(model, train_data_loader, optimizer, device):
    ...
    for batch_idx, batch in enumerate(train_data_loader):
        ...
        # Forward Pass
        loss, logits, se_loss, kws_loss = model(noisy, clean, target) 
    ...
    logging.info(f"Epoch Training Loss: {epoch_loss:.4f}, SE Loss: {epoch_se_loss:.4f}, KWS_Loss: {epoch_kws_loss:.4f}")

    return epoch_loss, epoch_se_loss, epoch_kws_loss
...
def cv(model, dataloader, device):
    ...
    return val_loss, epoch_se_loss, epoch_kws_loss
...
def main():
    ...
    elif last_checkpoint is None and configs['checkpoint_kws'] is not None:   
        # Load pre-trained KWS model if provided
        logging.info(f"Loading pre-trained KWS model from { configs['checkpoint_kws']}")
        kws_checkpoint = torch.load( configs['checkpoint_kws'], map_location='cpu', weights_only=True)
        model.kws_model.load_state_dict(kws_checkpoint, strict=False)
        if configs['checkpoint_se'] is not None:
            logging.info(f"Loading pre-trained SE model from { configs['checkpoint_se']}")
            se_checkpoint = torch.load(configs['checkpoint_se'], map_location='cpu', weights_only=True)
            model.se_model.load_state_dict(se_checkpoint)
        infos = {}
    else:
    ...
    #     threshold=0.01,
    # )
    # Freeze both SE and KWS models
    for param in model.module.se_model.parameters():
        param.requires_grad = False

    for param in model.module.kws_model.parameters():
        param.requires_grad = False


        # optimizer = optim.AdamW([
        #     {'params': model.module.se_model.parameters(), 'lr': 0.0001}
        # ])

    optimizer = optim.AdamW([
        {'params': model.module.se_model.parameters(), 'lr': 0.0001},  # SE의 학습률 낮추기
        {'params': model.module.kws_model.parameters(), 'lr': 0.0005}
    ])
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode='min',
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        train_loss, train_se_loss, train_kws_loss = train(model, train_data_loader, optimizer, device)
        train_losses.append(train_loss)
        train_se_losses.append(train_se_loss)
        train_kws_losses.append(train_kws_loss)
        
        cv_loss, cv_se_loss, cv_kws_loss = cv(model, cv_data_loader, device)
        ...
        if rank == 0:
            ...
            writer.add_scalars('Loss', {'Train': train_loss, 'Train SE': train_se_loss, 'Train KWS': train_kws_loss, 'Valid': cv_loss}, epoch)
