...
from wekws.model.classifier import (GlobalClassifier, LastClassifier,
                                    LinearClassifier)
from monet.model.subsampling import (LinearSubsampling1, Conv1dSubsampling1,
                                     NoSubsampling)
...
from monet.model.bc_resnet import BCResNets  # 🔥 추가
...
class KWSModel(nn.Module):
    ...
    def forward(
        ...
        if x.dim() == 3:
            x = x.unsqueeze(1)
        x = self.backbone(x)  # 🔥 BC-ResNet 통합
        x = self.classifier(x)
        x = self.activation(x)
        return x
...
def init_model(configs):
    ...
    elif backbone_type == 'bc_resnet':
        backbone = BCResNets(base_c=hidden_dim, num_classes=output_dim)
    else:
