...
from monet.model.subspectralnorm import SubSpectralNorm
...
class BCResNets(nn.Module):
    def __init__(self, base_c, num_classes=11):
        super().__init__()
    ...
    def forward(self, x):
        # MFCC Featrue : x = [B, T, D=80]
        if x.dim() == 3:
            x = x.permute(0, 2, 1).unsqueeze(1) # (B, 1, D=80, T)
