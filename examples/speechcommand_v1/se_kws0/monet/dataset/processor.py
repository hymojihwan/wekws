...
def parse_raw(data):
    """ Parse key/wav/txt from JSON line -> 직접 텐서 사용 """
    for sample in data:
        try:
            assert 'key' in sample and 'wav' in sample and 'txt' in sample
            
            key = sample['key']
            waveform = sample['wav']  # 🔥 여기서 바로 텐서를 가져옴
            ...
            example = dict(
                ...
                wav=waveform,  # 🔥 'wav' 대신 Tensor를 직접 저장
            )

            yield example

        except Exception as e:
            logging.warning(f"Skipping invalid data: {sample} ({str(e)})")
...
def filter(data, max_length=10240, min_length=10):
    ...
    for sample in data:
        try:
            ...
            # 🔥 Mono vs Stereo 대응
            if sample['wav'].dim() == 2:  # Stereo
                num_frames = sample['wav'].size(1) / sample['sample_rate'] * 100
            else:  # Mono (fallback)
                num_frames = sample['wav'].size(0) / sample['sample_rate'] * 100
